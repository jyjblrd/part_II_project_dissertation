...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \Require{$VisualWords_{ext}$: Set containing external keyframe's visual words}
        \Ensure{$Success$: Boolean value signaling if a merge is possible based on visual words}
        \State $(MergeScore_{ext},\ BestMatchKeyFrame) \gets \text{CalculateMergeScore}(VisualWords_{ext})$
        \State $(MergeScore_{baseline},\ \underbar{\ \ }\ ) \gets \text{CalculateMergeScore}(BestMatchKeyFrame \text{'s visual words})$
        \State $Success \gets MergeScore_{ext} \geq MergeScore_{baseline}$
    \end{algorithmic}
\end{algorithm}
...
The repository is split into 5 ROS packages.

\dirtree{%
    .1 orb\_slam3\_ros/\DTcomment{Core SLAM system}.
    .2 configs/\DTcomment{Configuration files for different cameras}.
    .2 include/\DTcomment{C++ headers for my SLAM system}.
    .2 src/\DTcomment{Source code for my SLAM system}.
    .3 orb\_slam3\_wrapper.cpp.
    .3 peer.cpp.
    .3 publish\_ros\_viz\_topics.cpp.
    .3 ros\_mono.cpp\DTcomment{Entrypoint for the SLAM system}.
    .2 orb\_slam3/\DTcomment{Heavily modified ORB-SLAM3 codebase}.
    .3 include/\DTcomment{C++ headers}.
    .3 src/.
}

\dirtree{%
    .1 controller/\DTcomment{Centeral management interface package}.
    .2 controller/.
    .3 my\_node.py\DTcomment{Source code for central management interface}.
}
...
\dirtree{%
    .1 .
    .2 src.
    .3 orb\_slam3\_ros/\DTcomment{Core SLAM system}.
    .5 configs/\DTcomment{Configuration files for different cameras}.
    .5 include/\DTcomment{C++ headers for my SLAM system}.
    .5 src/\DTcomment{Source code for my SLAM system}.
    .6 orb\_slam3\_wrapper.cpp.
    .6 peer.cpp.
    .6 publish\_ros\_viz\_topics.cpp.
    .6 ros\_mono.cpp\DTcomment{Entrypoint for the SLAM system}.
    .5 orb\_slam3/\DTcomment{Heavily modified ORB-SLAM3 codebase}.
    .6 include/\DTcomment{C++ headers}.
    .6 src/.
    .
    .3 evaluation/\DTcomment{Evaluation Jupyter Notebooks}.
    .4 evaluation.ipynb\DTcomment{Primary evaluation notebook}.
    .4 mocap\_calibrations.json\DTcomment{Coordinate frame offsets from an objects motion capture to camera coordinate frame}.
    .4 motion\_capture\_post\_processing.ipynb\DTcomment{Notebook to process and evaluate motion capture data from real-world experiments}.
    .3 interfaces/\DTcomment{Definitions for custom ROS message types}.
    .4 msg/\DTcomment{Custom message definitions}.
    % .5 ChangeCoordinateFrame.msg.
    % .5 IsLostFromBaseMap.msg.
    % .5 KeyFrameBowVector.msg.
    % .5 LoopClosureTriggers.msg.
    % .5 MapPoint.msg.
    % .5 MapToAttemptMerge.msg.
    % .5 NewKeyFrameBows.msg.
    % .5 NewKeyFrames.msg.
    % .5 Sim3Transform.msg.
    % .5 Sim3TransformStamped.msg.
    % .5 SuccessfullyMerged.msg.
    % .5 Uuid.msg.
    .4 srv/\DTcomment{Custom service definitions}.
    % .5 GetCurrentMap.srv.
    % .5 GetMapPoints.srv.
    .4 CMakeLists.txt\DTcomment{Build configuration for the package}.
    .3 motion\_controller/\DTcomment{Motion controller}.
    .3 webots\_sim/\DTcomment{Simulation package}.
    .4 launch/.
    .5 robot\_launch.py/\DTcomment{Simulation bridge configuration}.
    .4 webots\_sim/.
    .5 robot\_driver/\DTcomment{Webots to ROS bridge}.
    .4 worlds/\DTcomment{Simulation environments}.
    .5 city.wbt.
    .5 complete\_apartment.wbt.
    .4 local\_simulation\_server.py\DTcomment{Server-side Webots bridge script}.
    .3 tools/\DTcomment{Misc. scripts}.
    .2 .github/workflows/config.yml\DTcomment{CI/CD configuration}.
    .2 Dockerfile\DTcomment{Main dockerfile for the project}.
    .2 nvidia\_jetson.Dockerfile\DTcomment{Dockerfile for the NVidia Jetson}.
}
